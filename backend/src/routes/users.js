...
router.get('/:id', requireAuth, async (req, res) => {
  try {
    const userResult = await pool.query(
      'SELECT id, name, email, company, title, bio, school, avatar_url, created_at FROM users WHERE id = $1',
      [req.params.id]
    );
...
router.put('/me', requireAuth, async (req, res) => {
  const { name, company, title, bio, school, avatar_url, sns_links } = req.body;
  try {
    const result = await pool.query(
      'UPDATE users SET name=$1, company=$2, title=$3, bio=$4, school=$5, avatar_url=$6 WHERE id=$7 RETURNING id, name, email, company, title, bio, school, avatar_url',
      [name, company, title, bio, school, avatar_url, req.user.id]
    );
