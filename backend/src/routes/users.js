...
router.get('/:id', requireAuth, async (req, res) => {
  try {
    const userResult = await pool.query(
      'SELECT id, name, email, company, title, bio, school, hobby, node_color, avatar_url, created_at FROM users WHERE id = $1',
      [req.params.id]
    );
...
router.put('/me', requireAuth, async (req, res) => {
  const { name, company, title, bio, school, hobby, node_color, avatar_url, sns_links } = req.body;
  try {
    const result = await pool.query(
      'UPDATE users SET name=$1, company=$2, title=$3, bio=$4, school=$5, hobby=$6, node_color=$7, avatar_url=$8 WHERE id=$9 RETURNING id, name, email, company, title, bio, school, hobby, node_color, avatar_url',
      [name, company, title, bio, school, hobby, node_color || '#6366f1', avatar_url, req.user.id]
    );
